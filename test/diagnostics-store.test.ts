...
describe('diagnostics-store', () => {
  ...
  describe('update + merge', () => {
    it('single server, single URI → merge returns those diagnostics', () => {
      const store = update(empty(), 'vtsls', 'file:///a.ts', [diagA]);
      expect(merge(store, 'file:///a.ts')).toEqual([diagA]);
    });

    it('two servers, same URI → merge returns union', () => {
    ...
    it('update with empty array when last server removes URI entirely', () => {
      let store = update(empty(), 'vtsls', 'file:///a.ts', [diagA]);
      store = update(store, 'vtsls', 'file:///a.ts', []);
    ...
    it('merge on unknown URI returns empty array', () => {
      expect(merge(empty(), 'file:///unknown.ts')).toEqual([]);
    });

    it('replaces previous diagnostics for same server + URI', () => {
      let store = update(empty(), 'vtsls', 'file:///a.ts', [diagA]);
  ...
  describe('clearServer', () => {
    it('returns affected URIs and removes server from all', () => {
      let store = update(empty(), 'vtsls', 'file:///a.ts', [diagA]);
    ...
    it('is a no-op for server with no entries', () => {
      const store = update(empty(), 'vtsls', 'file:///a.ts', [diagA]);
  ...
  describe('immutability', () => {
    it('original store unchanged after update', () => {
      const before = empty();
    ...
    it('original store unchanged after clearServer', () => {
