...
describe('FlushScheduler', () => {
  it('calls onFlush after debounceMs', async () => {
    ...
    const scheduler = createFlushScheduler({ debounceMs: 100, maxWaitMs: 1000, onFlush });
  ...
  it('resets debounce on each notify', async () => {
    vi.useFakeTimers();
    const onFlush = vi.fn().mockResolvedValue(undefined);
    const scheduler = createFlushScheduler({ debounceMs: 100, maxWaitMs: 2000, onFlush });
  ...
  it('forces flush at maxWaitMs even when debounce keeps resetting', async () => {
    vi.useFakeTimers();
    const onFlush = vi.fn().mockResolvedValue(undefined);
    const scheduler = createFlushScheduler({ debounceMs: 100, maxWaitMs: 300, onFlush });
    ...
    await vi.advanceTimersByTimeAsync(50); // 300ms total → maxWait fires
    expect(onFlush).toHaveBeenCalledOnce();

    scheduler.dispose();
  });

  it('re-enters debounce cycle when notified during flush (not immediate)', async () => {
    vi.useFakeTimers();
    let flushCount = 0;
    const onFlush = vi.fn().mockResolvedValue(undefined);
    const scheduler = createFlushScheduler({ debounceMs: 100, maxWaitMs: 1000, onFlush });
  ...
  it('does not run concurrent flushes', async () => {
    ...
    const scheduler = createFlushScheduler({ debounceMs: 50, maxWaitMs: 1000, onFlush });

    scheduler.notify();
    await vi.advanceTimersByTimeAsync(50); // first flush starts (blocked)
    expect(onFlush).toHaveBeenCalledOnce();

    scheduler.notify(); // arrives during flush
    await vi.advanceTimersByTimeAsync(50); // debounce would fire, but flush is in progress
    expect(onFlush).toHaveBeenCalledOnce(); // still only one call

    resolveFlush(); // unblock first flush
    await vi.advanceTimersByTimeAsync(0); // let first flush complete
  ...
  it('recovers after onFlush throws', async () => {
    vi.useFakeTimers();
    const onFlush = vi.fn()
    ...
    scheduler.notify();
    await vi.advanceTimersByTimeAsync(100); // first flush fires and throws
    expect(onFlush).toHaveBeenCalledOnce();
  ...
  it('resets maxWait timer after flush completes', async () => {
    vi.useFakeTimers();
    const onFlush = vi.fn().mockResolvedValue(undefined);
    const scheduler = createFlushScheduler({ debounceMs: 100, maxWaitMs: 300, onFlush });
